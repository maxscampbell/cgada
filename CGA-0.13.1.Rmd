...
author: "Version 0.13.1 and Later"
output: 
  html_document:
    toc: true
    toc_float: true
...
dflist <- list(october5, october19, november2, november16)
...
suppressMessages(
  ...
  geom_point() + geom_text(hjust = 1, vjust = 1, check_overlap = FALSE) +
  xlab("Kills per Entry") + ylab("Coins per Entry") +
  labs(title = "Class Specializations: Kills per Entry vs. Coins per Entry") +
  geom_hline(yintercept = sum(cgada$Coins) / nrow(cgada), color = "black") +
  geom_vline(xintercept = sum(cgada$Kills) / nrow(cgada), color = "black") +
  theme(legend.position = "none") +
  annotate(geom = "text", x = 0.45, y = 24.5, label = "Coin-specialized", color = "black", alpha = 0.4) +
  annotate(geom = "text", x = 1.17, y = 24.5, label = "High Event", color = "black", alpha = 0.4) +
  annotate(geom = "text", x = 0.45, y = 11, label = "Low Event", color = "black", alpha = 0.4) +
  annotate(geom = "text", x = 1.17, y = 11, label = "Kill-specialized", color = "black", alpha = 0.4)
)
```
